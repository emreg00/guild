...
// to calculate Page Rank with initial ranks. Thus original license of the code
...
void
page_rank(const Graph& g, RankMap initial_rank_map, RankMap rank_map, Done done, 
          typename property_traits<RankMap>::value_type damping,
          typename graph_traits<Graph>::vertices_size_type n,
          RankMap2 rank_map2)
{
  ...
  while ((to_map_2 && !done(rank_map, g)) ||
    ...
    if (to_map_2) {
      detail::page_rank_step(g, rank_map, rank_map2, damping, n, category());
    } else {
      detail::page_rank_step(g, rank_map2, rank_map, damping, n, category());
    }
    to_map_2 = !to_map_2;
  }
          ...
          typename graph_traits<Graph>::vertices_size_type n)
{
  ...
  page_rank(g, initial_rank_map, rank_map, done, damping, n,
...
{
  page_rank(g, initial_rank_map, rank_map, done, damping, num_vertices(g));
}

template<typename Graph, typename RankMap>
inline void
page_rank(const Graph& g, RankMap initial_rank_map, RankMap rank_map)
{
  page_rank(g, initial_rank_map, rank_map, n_iterations(20));
}
